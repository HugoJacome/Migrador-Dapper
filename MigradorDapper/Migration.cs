...
namespace MigradorDapper
{
    public class Migration
    {
        ...
        public static string pathTarjetasCuentas = @"C:\Migracion\cardsAccounts.csv";
        public static string pathTarjetasOrden = @"C:\Migracion\cardsOrder.csv";
        public static string pathClientes = @"C:\Migracion\cliente.csv";
        public static string pathOrdenes = @"C:\Migracion\ordenes.csv";
        public static string pathAgencias = @"C:\Migracion\agencias.csv";
        public async Task<int> MigraAgencias()
        {
            List<Branch> branches = new List<Branch>();
            var agencias = await ccRepo.GetBranchs();

            using (var file = File.CreateText(pathAgencias))
            {
                file.WriteLine("InstitutionId;.Name;CoreIdentifier");
                foreach (var ag in agencias)
                {
                    ...
                    branches.Add(branch);
                    file.WriteLine(branch.InstitutionId + ";" + branch.Name + ";" + branch.CoreIdentifier);
                }
                file.WriteLine("Total Branches: " + branches.Count());
            }
            try
            {
        ...
        public async Task<int> MigraOrdenes()
        {
            ...
            using (var file = File.CreateText(pathOrdenes))
            {
                file.WriteLine("Quantity;CreationDate;ClosingDate");
                foreach (var orden in ordenes)
                {
                    ...
                    orders.Add(order);
                    file.WriteLine(order.Quantity + ";" + order.CreationDate + ";" + order.ClosingDate);
                }
                file.WriteLine("Total de ordenes: " + orders.Count());
            }
            try
            {
        ...
        public async Task<int> MigrarClientes29()
        {
            ...
            int branchId = await phiAdminRepo.GetBranchById(institutionID);
            List<List<Clients>> list = new List<List<Clients>>();

            using (var file = File.CreateText(pathClientes))
            {
                file.WriteLine("Identification;Name");
                foreach (var cli in clientes)
                {
                    ...
                    clients.Add(client);
                    file.WriteLine(client.Identification + ";" + client.Name);
                }
                int valorDivision = 5000;
                for (int i = 0; i < clients.Count; i += valorDivision)
                {
                    list.Add(clients.GetRange(i, Math.Min(valorDivision, clients.Count - i)));
                }
                file.WriteLine("Total Clients: " + clients.Count());
            }
            try
            {
        ...
        public async Task<int> MigraCuentasTarjetas29()
        {
            ...
            using (var file = File.CreateText(pathTarjetasCuentas))
            {
                file.WriteLine("CardId;AccountNumber");
                ...
                file.WriteLine("Total CardsAccounts: " + accounts.Count());
            }
            try
            {
        ...
        public async Task<int> MigraTarjetasOrden29()
        {
            ...
            using (var file = File.CreateText(pathTarjetasOrden))
            {
                file.WriteLine("CardNumber;ClientID;ClientIdentification;OrderId;CardState");
                ...
                file.WriteLine("CardsOrder: " + cards.Count());
            }
            try
            {
